...
void DotSceneLoader::parseDotScene(const String &SceneName, const String &groupName, SceneManager *yourSceneMgr,bool physics,NxOgre::Scene* physScene,Critter::RenderSystem* physRenderSys, SceneNode *pAttachNode, const String &sPrependNode)
{
	String msg = "[DotSceneLoader] Loading scene: "+SceneName;
	LogManager::getSingleton().logMessage(msg);
	// set up shared object values
	m_sGroupName = groupName;
	mSceneMgr = yourSceneMgr;
	m_bPhysics=physics;
	m_pPhysRenderSys=physRenderSys;
...
void DotSceneLoader::processCamera(TiXmlElement *XMLNode, SceneNode *pParent)
{
	...
	pCamera->setFOVy(Ogre::Radian(Ogre::Math::DegreesToRadians(fov)));
	...
	if(pElement)
		...
		pCamera->setPosition(pParent->getPosition());
...
void DotSceneLoader::processEntity(TiXmlElement *XMLNode, SceneNode *pParent)
{
	...
	try
	{
		if(m_bPhysics)
		{
			NxOgre::MeshManager* meshManager = NxOgre::MeshManager::getSingleton();
			if(isStatic)
			{
				//convert .mesh name to .nxs
				String phyxName=meshFile;
				for(int i=0;i<4;i++)
				{
					phyxName.pop_back();
				}
				phyxName.append("nxs");
				NxOgre::Mesh* mesh=meshManager->load("ogre://"+phyxName,name);

				Ogre::Vector3 pos = pParent->getPosition();
				Ogre::Quaternion q = pParent->getOrientation();
				NxOgre::Vec3 vec = NxOgre::Vec3::ZERO;
				vec.x=pos.x;
				vec.y=pos.y;
				vec.z=pos.z;

				NxOgre::Quat qu = NxOgre::Quat::IDENTITY;
				qu.w = q.w;
				qu.x = q.x;
				qu.y = q.y;
				qu.z = q.z;
				NxOgre::Matrix44 mat(vec,pParent->getOrientation());
				//add to scene geometry
				m_pPhysScene->createSceneGeometry(NxOgre::TriangleGeometryDescription(mesh),mat);
				//add ogre mesh to viualise
			...
			}else
			{
				if(id != "NO_PHYSX")
				{
					...
					pElement = XMLNode->FirstChildElement("Body");
					if(pElement)
					{
						bodyDescription = processBodyDescription(pElement,pParent);
					}else
					{
						//default body attributes
						bodyDescription.mMass = 20.0f;
						bodyDescription.mName = "PhysX"+pParent->getName();
						bodyDescription.mNode = pParent;
					}

					Ogre::Matrix4 final=Ogre::Matrix4::IDENTITY;
					final.makeTransform(pParent->getPosition(),pParent->getScale(),pParent->getOrientation());
					NxOgre::Matrix44 pose=NxOgre::Matrix44::IDENTITY;

					//pose.set(PhysicsSystem::GetSingleton()->convertOgreVec3(final.getTrans()));
					pose.set(PhysicsSystem::GetSingleton()->convertOgreQuat(final.extractQuaternion()));
					...
					Ogre::Vector3 Osc=pParent->getScale();
					NxOgre::Vec3 sc=NxOgre::Vec3::ZERO;
					sc.x=Osc.x;
					sc.y=Osc.y;
					sc.z=Osc.z;

					pose.set(vec.x,vec.y,vec.z);
					...
					Critter::Body* bod = m_pPhysRenderSys->createBody(NxOgre::ConvexDescription(mesh),pose,pEntity,bodyDescription);
					bod->setGroup(bodyDescription.mGroup);
				}else	//if PHYSX_NO_MESH then just make a Ogre Entity with no physX stuff
				{
					MeshManager::getSingleton().load(meshFile, m_sGroupName);
					pEntity = mSceneMgr->createEntity(name, meshFile);
					pEntity->setCastShadows(castShadows);
		...
		if(isStatic || (!isStatic && !m_bPhysics))
		{
			if(id == "NO_PHYSX")
			{
				MeshManager::getSingleton().load(meshFile, m_sGroupName);
...
void DotSceneLoader::processSound(TiXmlElement *XMLNode, SceneNode *pParent)
{
	String name = getAttrib(XMLNode, "name");
	String fileName =	getAttrib(XMLNode, "fileName");
	bool bStreamed = getAttribBool(XMLNode, "streamed",false);
	bool bLooping = getAttribBool(XMLNode, "looping",false);
	float size = getAttribReal(XMLNode,"size",1.0f);

	SoundSystem::GetSingleton()->load3dSound(fileName.c_str(),bStreamed);
	SoundNode* sound=0;
	if(name != "")
		sound = SoundNodeManager::GetSingleton()->createSoundNode(name,fileName,bLooping,size);
	else
		sound = SoundNodeManager::GetSingleton()->createSoundNode(fileName,bLooping,size);
	assert(sound);	//if sound is null
	pParent->attachObject(sound);
}

void DotSceneLoader::processTrigger(TiXmlElement *XMLNode, SceneNode *pParent)
{
	String shape = getAttrib(XMLNode,"shape");
	Real size = getAttribReal(XMLNode,"size");
	String luaObj = getAttrib(XMLNode,"luaObj");
	String luaFile = getAttrib(XMLNode,"luaFile");

	TriggerManager::TriggerShape sh;	//shape of trigger volume
	float si;							//size of trigger volume
	luabind::object obj;				//the lua obj
	if(shape == "cube")
		sh = TriggerManager::TriggerShape::Cube;
	else
		sh = TriggerManager::TriggerShape::Sphere;

	si = size;
	LuaScriptManager::GetSingleton()->doFile(luaFile.c_str());
	luabind::object g = luabind::globals(LuaScriptManager::GetSingleton()->getLuaState());	//the states we declare are in global table in Lua
	obj = g[luaObj.c_str()];	//retrieve the state from the Lua global table
	assert(obj.is_valid());		//this should be valid, check for typoes in Lua file
	TriggerManager::GetSingleton()->createTriggerVolume(sh,si,obj,pParent->getPosition());
}

Critter::BodyDescription DotSceneLoader::processBodyDescription(TiXmlElement *XMLNode, SceneNode *pParent)
{
	Real angularDamping =  getAttribReal(XMLNode, "angularDamping",0.05);
	Real linearDamping = getAttribReal(XMLNode, "linearDamping",0.0);
	Real density = getAttribReal(XMLNode, "density",0.0);
	Real mass = getAttribReal(XMLNode, "mass",1.0);
	int group = getAttribInt(XMLNode,"group",0);

	//DynamicRidigBody flags
	bool DisableGravity = getAttribBool(XMLNode,"disableGravity");
	bool FreezePosX = getAttribBool(XMLNode,"freezePosX");
	bool FreezePosY = getAttribBool(XMLNode,"freezePosY");
	bool FreezePosZ = getAttribBool(XMLNode,"freezePosZ");
	bool FreezePos = getAttribBool(XMLNode,"freezePos");
	bool FreezeRotX = getAttribBool(XMLNode,"freezeRotX");
	bool FreezeRotY = getAttribBool(XMLNode,"freezeRotY");
	bool FreezeRotZ = getAttribBool(XMLNode,"freezeRotZ");
	bool FreezeRot = getAttribBool(XMLNode,"freezeRot");

	Critter::BodyDescription body;
	body.mAngularDamping=angularDamping;
	body.mLinearDamping=linearDamping;
	body.mDensity=density;
	body.mMass=mass;
	body.mGroup=group;
	body.mName="PhysX"+pParent->getName();
	body.mNode = pParent;

	if(DisableGravity)
		body.mDynamicRigidbodyFlags |= NxOgre::DynamicRigidbodyFlags::DisableGravity;
	if(FreezePosX)
		body.mDynamicRigidbodyFlags |= NxOgre::DynamicRigidbodyFlags::FreezePositionX;
	if(FreezePosY)
		body.mDynamicRigidbodyFlags |= NxOgre::DynamicRigidbodyFlags::FreezePositionY;
	if(FreezePosZ)
		body.mDynamicRigidbodyFlags |= NxOgre::DynamicRigidbodyFlags::FreezePositionZ;
	if(FreezePos)
		body.mDynamicRigidbodyFlags |= NxOgre::DynamicRigidbodyFlags::FreezePosition;
	if(FreezeRotX)
		body.mDynamicRigidbodyFlags |= NxOgre::DynamicRigidbodyFlags::FreezeRotationX;
	if(FreezeRotY)
		body.mDynamicRigidbodyFlags |= NxOgre::DynamicRigidbodyFlags::FreezeRotationY;
	if(FreezeRotZ)
		body.mDynamicRigidbodyFlags |= NxOgre::DynamicRigidbodyFlags::FreezeRotationZ;
	if(FreezeRot)
		body.mDynamicRigidbodyFlags |= NxOgre::DynamicRigidbodyFlags::FreezeRotation;
